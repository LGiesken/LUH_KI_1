...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
    # Return new state with tupel(x and y coordinates) and four bools (true if corner visited)
    @staticmethod
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if not all corners have been visited return false
...
def cornersHeuristic(state, problem):
    ...
    if not state[1][0]:
        manhattenSteps.append(
            (abs(corners[0][0]-state[0][0])+(abs(corners[0][1]-state[0][1]))))
    if not state[1][1]:
        manhattenSteps.append(
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    """
    distances contains the distance between the current position and every
    food in the grid. The function returns the maximum of distances
    or 0 if distances is empty
    """

    distances = [mazeDistance(position, foodPosition, problem.startingGameState)
